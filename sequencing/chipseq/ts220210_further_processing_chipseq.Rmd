...
# Prepare output 
output_dir <- "ts220210_further_processing_chipseq"
...
# Read counts
counts <- read_tsv("ts220210_chipseq/deeptools/deeptools_bam_coverage.tab")
...
metadata <- tibble(sample = samples) %>%
  mutate(target = case_when(str_detect(sample, "antiGFP") ~ "GFP",
                            str_detect(sample, "antiKI67") ~ "Ki67",
                            T ~ "input"),
         timepoint = ifelse(str_detect(sample, "24h"),
                            "24h", "0h")) %>%
  mutate(target = factor(target, levels = c("input", "Ki67", "GFP")),
  ...
  mutate(
    ...
    HCT116_Ki67_24h = 
      log2((HCT116_Ki67_24h + 1) / 
             (HCT116_input_24h + 1)),
    HCT116_GFP_0h = 
      log2((HCT116_GFP_0h + 1) / 
             (HCT116_input_0h + 1)),
    HCT116_GFP_24h = 
      log2((HCT116_GFP_24h + 1) / 
             (HCT116_input_24h + 1))) %>%
...
norm_diff <- norm %>%
  mutate(HCT116_Ki67_0h = HCT116_Ki67_0h - HCT116_Ki67_24h,
         HCT116_GFP_0h = HCT116_GFP_0h - HCT116_GFP_24h) %>%
...
norm %>%
  ggplot(aes(x = HCT116_GFP_0h, y = HCT116_GFP_24h)) +
  geom_point(alpha = 0.1) +
  geom_abline(intercept = 0, slope = 1, col = "red") +
  ggtitle("GFP antibody") +
  theme_bw() +
  theme(aspect.ratio = 1)
...
norm %>%
  ggplot(aes(x = HCT116_Ki67_0h, y = HCT116_GFP_0h)) +
  geom_point(alpha = 0.1) +
  geom_abline(intercept = 0, slope = 1, col = "red") +
  ggtitle("Ki-67 antibody") +
  theme_bw() +
...
norm %>%
  ggplot(aes(x = HCT116_Ki67_24h, y = HCT116_GFP_24h)) +
  geom_point(alpha = 0.1) + 
  geom_abline(intercept = 0, slope = 1, col = "red") +
  ggtitle("GFP antibody") +
  theme_bw() +
