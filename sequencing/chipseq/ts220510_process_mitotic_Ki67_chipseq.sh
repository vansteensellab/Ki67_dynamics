...
dir_chipseq="ts220510_chipseq"
mkdir $dir_chipseq

# List files
samples_dir="/shared/gcf/c.moene/6851/fastq_files/"
samples=$(ls $samples_dir/6851_[AGCT]*.fastq.gz)
...
for p1 in $samples; do
  ...
  ../ts200921_LaminaNucleolus_AdditionalExperiments/bin/mapping/mapping_bwa.sh \
    ...
    -d -c 8 -f $dir_chipseq/mapping/${base}_bwa.log &
done
