...
# Input data
results_dir <- c("../ts201102_E1349_confocal_synchronization/ts201102_Ki67_depletion/ts201102_HCT116_Ki67_AID24_knockdown_analysis",
                 "../ts201207_Exxx_confocal_various_experiments/ts210316_Ki67_depletion/ts201207_HCT116_Ki67depl_analysis/")

# Prepare output directory
output_dir <- "ts210609_ki67_intensity_following_auxin_depletion"
...
metadata <- tibble(cell_statistics = dir(results_dir, recursive = T, 
  ...
  mutate(sample = str_remove(basename(cell_statistics),
         ...
         sample = str_remove(sample, "HCT116_Ki67AID_"),
  ...
  mutate(condition = recode(condition,
         ...
         replicate = case_when(grepl("r1_", cell_statistics) ~ "r2",
...
```{r plot targets, fig.width = 5, fig.height = 7}

# Plot intensity per cell
value_norm_limit <- 0.5
...
tib_gather %>%  
  ...
  geom_boxplot(fill = NA, col = "black", outlier.shape = NA) +
  facet_grid(key ~ replicate, scales = "free_y") +
  xlab("") +
  ylab("Intensity (A.U.)") +
  theme_bw() +
  theme(aspect.ratio = 1,
...
Ki67-AID results in Ki67 depletion following auxin addition. Ki67 antibody 
binding is only partially reduced.
